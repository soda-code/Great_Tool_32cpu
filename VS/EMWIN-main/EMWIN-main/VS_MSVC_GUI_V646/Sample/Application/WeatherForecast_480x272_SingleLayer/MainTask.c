...
static GUI_EMSCRIPTEN_DATE _aTime[4];
#endif
...
static void _cbText(WM_MESSAGE * pMsg) {
...
#ifdef __EMSCRIPTEN__
  char                        ac[128];
  const GUI_EMSCRIPTEN_DATE * pDate;
#endif
  ...
  case WM_PAINT:
    ...
    for (i = 0; i < GUI_COUNTOF(aColor); i++) {
      ...
      GUI_SetFont(&GUI_Font20_AA4);
#ifdef __EMSCRIPTEN__
      pDate = &_aTime[pData->IndexCity];
      sprintf(ac, "%02d:%02d %s %d %s", pDate->Hour, pDate->Minute, _aWeekdays[pDate->Weekday - 1], pDate->Day, _aMonths[pDate->Month - 1]);
      GUI_DispString(ac);
#else
...
*       _cbTimerPollDate
*/
static void _cbTimerPollDate(GUI_TIMER_MESSAGE * pTM) {
  int i;
  
  //
  // Retrieve new date for all cities
  //
  for (i = 0; i < GUI_COUNTOF(_aTime); i++) {
    GUI_EMSCRIPTEN_GetNowTimezone(&_aTime[i], _aCity[i].sTimezone);
  }
  //
  // Restart timer
  //
  GUI_TIMER_SetPeriod(pTM->hTimer, POLL_DATE_PERIOD);
...
*       _CreateDateTimer
*/
static void _CreateDateTimer(void) {
  GUI_TIMER_HANDLE hTimer;

  hTimer = GUI_TIMER_Create(_cbTimerPollDate, 0, 0, 0);
