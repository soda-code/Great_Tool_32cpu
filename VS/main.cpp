...
std::string S_FILE_PATH = "OUT/0100040s.bys"; // 替换为你的文件路径
std::ifstream FileHandler::S_FILE_DATA; // 定义静态成员变量
            ...
            for (const auto& entry : fs::directory_iterator(dir_path)) 
            {
                ...
                if (dot_pos != std::string::npos && dot_pos < filename.size() - 1)
                {
                    ...
                    if ((extension.size() > 1)&&(filename.size()>extension.size()&&(filename.size()>5)))
                    {
                        ...
                        if (filename[filename.size() - extension.size() - 2] == 's')
                        {
                            str1 += filename;
                            S_FILE_PATH = str1;
                            if (FileHandler::OPEN_S_DATA_File(S_FILE_PATH))
                            {
                                return_get =  S_file_csv();
                            }
                        }
