...
#elif STORE_DOUBLE
const char* M_FILE_name = { "\"Time\",\"IPAP\",\"EPAP\",\"Flow\",\"OAI\",\"CAI\",\"HI\",\"SI\",\"FL\",\"RERA\",\"CSR\",\"LVT\",\"VT\",\"MV\",\"IE\",\"RR\",\"SPOP2\",\"PR\",\"End\"\n" };
#endif
// 假设你有一个数据结构来存储数据
struct Data_m_file
{
...
#elif STORE_DOUBLE
    unsigned char IPAP[2]; //当前压力    1  单水平压力=Value*0.1;
    unsigned char EPAP[2];//当前压力    1  单水平压力=Value*0.1; 
#endif
...
#define  numBytes  sizeof(Data_m_file)
int m_file_csv(void)
{
    ...
    for (size_t tick_fila = 0; ;tick_fila++)
    {
        ...
        memcpy(&M_FILE_deal, &ch_get, numBytes);
        memset(&ch_get, 0, numBytes);
            ...
            file_M_DATA_csv << (int)M_FILE_Time.year + 2000 << "-" << (int)M_FILE_Time.month << "-" << (int)M_FILE_Time.day << " "\
            << (int)M_FILE_Time.hour << ":" << (int)M_FILE_Time.min << ":" << (int)M_FILE_Time.sec << ","\
            << ((int)M_FILE_deal.IPAP[1]<<8| M_FILE_deal.IPAP[0]) << "," << ((int)M_FILE_deal.EPAP[1] << 8 | M_FILE_deal.EPAP[0]) << ","\
            << (int)M_FILE_deal.Flow << "," << (int)M_FILE_deal.OAI << ","<< (int)M_FILE_deal.CAI << "," << (int)M_FILE_deal.HI << "," \
            << (int)M_FILE_deal.SI << "," << (int)M_FILE_deal.FL << "," << (int)M_FILE_deal.RERA << "," << (int)M_FILE_deal.CSR << ","\
            << ((int)M_FILE_deal.LVT[1] << 8 | M_FILE_deal.LVT[0]) << "," << ((int)M_FILE_deal.VT[1] << 8 | M_FILE_deal.VT[0]) << "," << (int)M_FILE_deal.MV << "," \
            << (int)M_FILE_deal.IE << "," << (int)M_FILE_deal.RR << "," << (int)M_FILE_deal.SPO2 << "," \
            << (int)M_FILE_deal.PR << "," << (int)M_FILE_deal.End << ","  \
            << "\n";
        #endif
